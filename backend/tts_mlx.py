...
# -------------------------------------------------------------------
# Shared loader so that other modules (e.g. FastAPI services) can
# reuse the already-initialized TTS components. Heavy checkpoints
# are loaded only once per process and cached in memory.
# -------------------------------------------------------------------
_cached_tts = None  # type: ignore
_cached_mimi = None  # type: ignore
_cached_cfg_is_no_text = None  # type: ignore
_cached_cfg_is_no_prefix = None  # type: ignore
_cached_cfg_coef_conditioning = None  # type: ignore
...
def load_tts_model(
    *,
    ...
    """Load the MLX TTS model once and return cached components.

    Returns
    -------
    tuple(tts_model, mimi, cfg_is_no_text, cfg_is_no_prefix, cfg_coef_conditioning)
    """
    global _cached_tts, _cached_mimi, _cached_cfg_is_no_text, _cached_cfg_is_no_prefix, _cached_cfg_coef_conditioning

    # Return immediately if we've already loaded a model with identical settings
    if _cached_tts is not None:
        return (
            _cached_tts,
            _cached_mimi,
            _cached_cfg_is_no_text,
            _cached_cfg_is_no_prefix,
            _cached_cfg_coef_conditioning,
        )

    mx.random.seed(299792458)

    log("info", "retrieving checkpoints for cached loader")

    raw_config_path = hf_get("config.json", hf_repo)
    with open(raw_config_path, "r") as fobj:
    ...
    if quantize is not None:
        log("info", f"quantizing model to {quantize} bits")
    ...
    log("info", f"loading the text tokenizer from {tokenizer_path}")
    ...
    _cached_tts = tts_model
    _cached_mimi = mimi
    _cached_cfg_is_no_text = cfg_is_no_text
    _cached_cfg_is_no_prefix = cfg_is_no_prefix
    _cached_cfg_coef_conditioning = cfg_coef_conditioning

    log("info", "TTS model loaded and cached")
